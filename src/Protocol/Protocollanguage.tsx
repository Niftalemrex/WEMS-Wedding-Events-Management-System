// src/Protocol/ProtocolLanguage.tsx
import React, { useEffect } from "react";
import { useAppSettings } from "../Contexts/AppSettingsContext";
import type { Language } from "../Contexts/AppSettingsContext";

import "./ProtocolLanguage.css"; // can reuse Vendor/Admin CSS
...
const ProtocolLanguage: React.FC = () => {
  const { language, setLanguage, isRTL, t } = useAppSettings();

  // Load stored protocol language
  useEffect(() => {
    const storedLang = localStorage.getItem("protocol-language") as Language | null;
    if (storedLang && LANGUAGES.includes(storedLang)) setLanguage(storedLang);
  }, [setLanguage]);

  const handleChangeLanguage = (code: Language) => {
    setLanguage(code);
    localStorage.setItem("protocol-language", code);
  };

  return (
    <div className={`protocol-language ${isRTL ? "rtl" : ""}`}>
...
export default ProtocolLanguage;
