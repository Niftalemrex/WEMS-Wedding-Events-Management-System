...
const ManagerLanguage: React.FC = () => {
  const { language, setLanguage, isRTL, t } = useAppSettings();

  // Load stored language
  useEffect(() => {
    const storedLang = localStorage.getItem("manager-language") as Language | null;
    if (storedLang && LANGUAGES.includes(storedLang)) setLanguage(storedLang);
  }, [setLanguage]);

  const handleChangeLanguage = (code: Language) => {
    setLanguage(code);
    localStorage.setItem("manager-language", code);
  };

  return (
    <div className={`manager-language ${isRTL ? "rtl" : ""}`}>
