import React, { useState } from "react";
import "./ManagerSidebar.css";
import {
  FiGrid,
  FiCalendar,
  FiUsers,
  FiClipboard,
  FiDollarSign,
  FiBox,
  FiCheckSquare,
  FiUser,
  FiGlobe,
  FiSun,
  FiLogOut,
} from "react-icons/fi";
import type { ManagerPage } from "./ManagerDashboard";
import { useAppSettings } from "../Contexts/AppSettingsContext";

interface UserProfile {
  name: string;
  email: string;
  avatar?: string;
}

interface ManagerSidebarProps {
  activePage: ManagerPage;
  setActivePage: React.Dispatch<React.SetStateAction<ManagerPage>>;
  userProfile: UserProfile;
}

const ManagerSidebar: React.FC<ManagerSidebarProps> = ({
  activePage,
  setActivePage,
  userProfile,
}) => {
  const [expandedSettings, setExpandedSettings] = useState(false);
  const { t } = useAppSettings(); // translation function

  // Main navigation items
  const mainMenuItems: { id: ManagerPage; labelKey: string; icon: React.ReactNode }[] = [
    { id: "overview", labelKey: "overview", icon: <FiGrid /> },
    { id: "manage-events", labelKey: "manageEvents", icon: <FiCalendar /> },
    { id: "manage-guests", labelKey: "manageGuests", icon: <FiUsers /> },
    { id: "vendor-coordination", labelKey: "vendorCoordination", icon: <FiClipboard /> },
    { id: "task-checklist", labelKey: "taskChecklist", icon: <FiCheckSquare /> },
    { id: "budget", labelKey: "budget", icon: <FiDollarSign /> },
    { id: "inventory", labelKey: "inventory", icon: <FiBox /> },
  ];

  // Settings submenu items
  const settingsMenuItems: { id: ManagerPage; labelKey: string; icon: React.ReactNode }[] = [
    { id: "profile", labelKey: "profile", icon: <FiUser /> },
    { id: "language", labelKey: "language", icon: <FiGlobe /> },
    { id: "appearance", labelKey: "appearance", icon: <FiSun /> },
  ];

  // Logout action
  const handleLogout = () => {
    console.log("Manager logging out...");
    // implement actual logout logic
  };

  return (
    <aside className="manager-sidebar">
      {/* Profile Header */}
      <div className="sidebar-header">
        <div className="profile-card">
          <div className="profile-avatar">
            {userProfile.avatar ? (
              <img src={userProfile.avatar} alt={userProfile.name} />
            ) : (
              <FiUser size={24} />
            )}
          </div>
          <div className="profile-info">
            <h3 className="profile-name">{userProfile.name}</h3>
            <p className="profile-email">{userProfile.email}</p>
          </div>
        </div>
      </div>

      {/* Main Menu */}
      <nav className="sidebar-menu">
        <ul>
          {mainMenuItems.map((item) => (
            <li
              key={item.id}
              className={activePage === item.id ? "active" : ""}
              onClick={() => setActivePage(item.id)}
            >
              <span className="menu-icon">{item.icon}</span>
              <span className="menu-label">{t(item.labelKey)}</span>
            </li>
          ))}
        </ul>
      </nav>

      {/* Settings Section */}
      <div className="settings-section">
        <div
          className={`settings-toggle ${expandedSettings ? "expanded" : ""}`}
          onClick={() => setExpandedSettings(!expandedSettings)}
        >
          <span className="settings-label">{t("settings")}</span>
        </div>

        {expandedSettings && (
          <ul className="settings-submenu">
            {settingsMenuItems.map((item) => (
              <li
                key={item.id}
                className={activePage === item.id ? "active" : ""}
                onClick={() => setActivePage(item.id)}
              >
                <span className="menu-icon">{item.icon}</span>
                <span className="menu-label">{t(item.labelKey)}</span>
              </li>
            ))}
            <li className="logout-button" onClick={handleLogout}>
              <FiLogOut className="menu-icon" />
              <span>{t("logout")}</span>
            </li>
          </ul>
        )}
      </div>
    </aside>
  );
};

export default ManagerSidebar;
